...
class Post
{
	...
	public function create_post($userid, $data, $files)
	{
...

		{
			...
			$query = "insert into posts (userid,postid,post,image,is_image,parent) values('$userid','$postid','$post','$myimage','$is_image','$parent')";
	...
	{
			$query = "select * from posts where parent = 0 and userid = '$id' order by id desc limit 50";
			$DB = new Database();
	...
	{
			$query = "select * from posts where parent = '$id' order by id asc limit 50";
			$DB = new Database();
		    $result = $DB->read($query);
		    if($result)
		    {
	...
	public function i_own_post($postid, $NFoster_userid)
	{
		...
		if(!is_numeric($postid))
		{
		   ...
		   $result = $DB->read($query);
		    if(is_array($result))
		    {
		    	if($result[0]['userid'] == $NFoster_userid)
		    	{
		    		return true;
		    	}
	...
	public function like_post($id,$type,$NFoster_userid)
	{
		   ...
		   if(is_array($result))
		   {
			...
			if(!in_array($NFoster_userid, $user_ids))
			{
				$arr["userid"] = $NFoster_userid;
		...
		//increment post table
			...
			{
			  $key = array_search($NFoster_userid, $user_ids);
			  unset($likes[$key]); 
			  $likes_string = json_encode($likes);
